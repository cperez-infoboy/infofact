...
PHASE_PROMPTS: dict[str, str] = {
    "requirements": (
        ...
        "iteres get_requirement para datos que el listado ya devuelve. Los "
        "requerimientos viven en la base de datos del backend, fuera del "
        "sandbox: nunca los busques en el filesystem (no hay archivos .db "
        "ni .sqlite accesibles) y no uses execute para salir del directorio "
        "del proyecto — el workspace solo contiene los documentos fuente. "
        ...
        "\n"
        "## Política de delegación en tandas\n"
        "Cuando una operación requiera varias tareas `task` (por ejemplo, "
        "capturar los documentos del proyecto en lotes):\n"
        "- Divide el trabajo en tareas ACOTADAS: cada `task` cubre 2 o 3 "
        "secciones/documentos, nunca todo el alcance de una vez.\n"
        "- Dispara como máximo 1 o 2 `task` por mensaje y espera sus "
        "resultados antes de seguir: cada tanda completada deja un "
        "checkpoint del orquestador y evita repetir trabajo ya hecho si el "
        "proceso se corta.\n"
        "- Tras cada tanda, verifica el resultado en el store "
        "(`capture_status`, `list_requirements`) antes de delegar la "
        "siguiente.\n"
